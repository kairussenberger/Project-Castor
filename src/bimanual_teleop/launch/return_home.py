...
REANCHOR_TOL = 0.50   # rad (~29°): max settle-vs-home error we still RE-ANCHOR (covers normal drift like a 13° j4; rejects a tangled/propped/wrong arm ~180°)
...
def home_one(side: str, rig: dict, map_file, rate_deg_s: float, reanchor: bool) -> bool:
    neutral = np.asarray(rig["arms"][side]["neutral_q"], dtype=float)
    channel = rig["arms"][side]["can_channel"]
    _tol = rig.get("hardware", {}).get("engage_pose_tol", 0.15)
    engage_tol = float(np.max(_tol))   # scalar OR per-joint list → worst-case, for the log line only
...
def main() -> int:
    ...
    log.info("return-home: sides=%s rate=%.0f°/s reanchor=%s map=%s",
             sides, args.rate, not args.no_reanchor, map_file)

    ok = True
    try:
        for side in sides:
            ok = home_one(side, rig, map_file, args.rate, not args.no_reanchor) and ok
